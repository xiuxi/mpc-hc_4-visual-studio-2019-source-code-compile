...
int load_1 = false;
#include "D:\gitHub\java_ubuntu_windows\python_pt\python call dll c cpp\java_cpp1_c_export_32\java_cpp1\java_cpp1.h"
void task1(std::string str)
{
    try {
        const char* cstr = str.c_str();

        hello((char*)cstr, str.length(), 1);
    }
    catch (...) {
    }
}
extern std::string srt_path;
bool en_call = false;
bool zh_call = false;
std::unique_ptr<std::thread> en_thread_;
std::unique_ptr<std::thread> zh_thread_;
#include <process.h>
std::string srt_en_;
std::string srt_zh_;
void en_f2(void* param)
{
    try {
        const char* cstr = srt_en_.c_str();
        speach_en((char*)cstr, srt_en_.length(), 1, 4, 40);
    }
    catch (...) {}
    en_call = true;
}
void zh_f2(void* param)
{
    try {
        const char* cstr = srt_zh_.c_str();
        speach_en((char*)cstr, srt_zh_.length(), 1, 4, 100);
    }
    catch (...) {}
    zh_call = true;

}
void func2(std::string str, int entry) {
    try {

        en_call = false;
        zh_call = false;
        std::size_t pos = str.find("`");
        std::string srt_en = str;
        srt_en_ = srt_en;
        std::string srt_zh = "";
        srt_zh_ = "";
        if (pos > 0) {
            //str = str.substr(0, pos);
            srt_zh = str.substr(0, pos);
            srt_en = str.substr(pos + 1);
            srt_zh_ = srt_zh;
            srt_en_ = srt_en;
        ...
        if (true) {
            ...
            //pFile2 = fopen("C:\\mpc-hc\\srt_tmp\\srt_line.txt", "a+");
            pFile2 = fopen("C:\\mpc-hc\\srt_tmp\\srt_line.txt", "w+");
            fprintf(pFile2, srt_path.c_str());
            fprintf(pFile2, "\n");
            fprintf(pFile2, std::to_string(entry).c_str());
            fprintf(pFile2, "\n");
            fprintf(pFile2, srt_zh.c_str());
            fprintf(pFile2, "\n");
            fclose(pFile2);

            pFile2 = fopen("C:\\mpc-hc\\srt\\srt_line.txt", "w+");
            fprintf(pFile2, srt_path.c_str());
            fprintf(pFile2, "\n");
            fprintf(pFile2, std::to_string(entry).c_str());
            fprintf(pFile2, "\n");
        ...
        if (load_1 == false) {
            load_1 = true;
            //LoadLibrary(L"D:\gitHub\java_ubuntu_windows\python_pt\python call dll c cpp\java_cpp1_c_export_32\java_cpp1\Debug\java_cpp1.dll");
        ...
        if (srt_en.length() > 0) {
            try {
                en_call = false;
                auto en_f1 = [&]() {
                    //en_thread_.reset(new std::thread([&] {
                    try {
                        const char* cstr = srt_en.c_str();
                        speach_en((char*)cstr, srt_en.length(), 1, 4, 40);
                    }
                    catch (...) {
        ...
        if (srt_zh.length() > 0) {
            try {
                zh_call = false;
                auto zh_f = [&]() {
                    //zh_thread_.reset(new std::thread([&] {
                    try {
                        const char* cstr = srt_zh.c_str();
                        //speach_en((char*)cstr, srt_zh.length(), 1, 4, 100);
                    }
                    catch (...) {
    ...
    FILE* pFile2;
    pFile2 = fopen("d:\\test1.txt", "a");
    str.append("\n");
    fprintf(pFile2, str.c_str());
    fclose(pFile2);
}
int prev_size = 0;
CSubtitle* CRenderedTextSubtitle::GetSubtitle(int entry)
{
    ...
    std::string stdstr = CW2A(str);
    //this->count
    
    //for (auto t : this->GetCount()) {

    //}
    auto data1 = this->GetAt(0);

    auto size1 = this->GetCount();
    std::string all_srt = "";
    try {
        if (false && size1 != prev_size) {
            prev_size = size1;
            FILE* pFile2;


            //pFile2 = fopen("C:\\mpc-hc\\srt_tmp\\srt_lines.txt", "a+");
            pFile2 = fopen("C:\\mpc-hc\\srt_tmp\\srt_lines.txt", "w+");
            std::string all;
            for (int i = 0; i < size1; i++) {
                CStringW srt = GetStrW(i, true);
                std::string srt_text = CW2A(srt);
                all.append(srt_text);
                all.append("\n");
                //fprintf(pFile2, srt_text.c_str());
                //fprintf(pFile2, "\n");

                // all_srt.append(srt_text);
                // all_srt.append("\n");
                //auto srt = this->GetAt(i);
                //auto stdstr = CW2A(srt);
                //srt->str();
                //all_srt.append(srt);
                //i + 1;
            }
            fprintf(pFile2, all.c_str());
            fclose(pFile2);
